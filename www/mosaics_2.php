...
function display_index($entityManager) {
    global $cwd;
    ...
    // get mosaics UUIDs for the project
    $mosaic_ids = array();
    $mosaics = $entityManager->getRepository('Project')
        ->findOneBy(array('uuid' => $_GET['project']))->getMosaics();
    foreach ($mosaics as $mosaic) {
        $mosaic_ids[] = $mosaic->getUuid()->toString();
    }
    return $mosaic_ids;
...
function get_mosaic_card($entityManager) {
    ...
    $mosaic = $entityManager->getRepository('Mosaic')
        ->findOneBy(array('uuid' => $_GET['mosaicUuid']));
    ...
    $mosaic_info['thumbnail'] = $thumbnail_filename;
    $preview_filename = "mosaics/{$mosaic_owner_id}/{$filename_base}_preview.png";
    $mosaic_info['preview'] = $preview_filename;
